# Modifiche fisiche pendenti — mazzi pol

Tracker delle modifiche fatte nei txt ma **non ancora applicate alle deckbox fisiche**.
Spunta la casella quando lo swap è fatto. Le posizioni ("dove trovarla") vengono dal
CSV collezione del 2026-07-02 — se le carte si spostano, fidati dei box reali.

_Aggiornato: 2026-07-02_

## first_sliver

- [ ] FUORI: Icon of Ancestry → DENTRO: **Assassin's Trophy** (Ff gift box, ACR — 2 copie lì)
- [ ] FUORI: Bloodline Bidding → DENTRO: **Path to Exile** (bulk, ECC o SOC)
- [ ] FUORI: Harsh Mercy → DENTRO: **Swords to Plowshares** (bulk, SOC — ultima copia libera)

## shroofus (La cosa verde)

- [ ] FUORI: Defense Grid → DENTRO: **Beast Within** (box Spiderman, EOC)

## edgar_markov

- [ ] FUORI: Zombify → DENTRO: **Go for the Throat** (bulk, LTC)

## sam_frodo (Disturbi alimentari)

- [ ] FUORI: Talisman of Unity → DENTRO: **Infernal Grasp** (bulk, ECC o SOC)
- [ ] FUORI: Talisman of Hierarchy → DENTRO: **Hero's Downfall** (Foundation o Avatar bundle, FDN/VOW — 4 copie in giro)

## mimeoplasm

- [ ] FUORI: Sultai Banner → DENTRO: **Assassin's Trophy** (bulk, ECC o SOC)
- [ ] FUORI: Auroral Procession → DENTRO: **Hero's Downfall** (Foundation o Avatar bundle)
- [ ] FUORI: Crop Rotation → DENTRO: **Toxic Deluge** (2026-07-31, taglio di bracket:
  Crop Rotation era l'unico GC, portava il mazzo a B3 basso — ora B2 pulito)
- [ ] FUORI: Death's Shadow → DENTRO: **Arcane Signet** (2026-07-31, voltron/protezione)
- [ ] FUORI: Myr Convert → DENTRO: **Sol Ring** (2026-07-31, voltron/protezione)
- [ ] FUORI: Tolarian Winds → DENTRO: **Whispersilk Cloak** (2026-07-31, voltron/protezione)
- [ ] FUORI: Heritage Reclamation → DENTRO: **Command Tower** (2026-07-31, voltron/protezione)
- [ ] FUORI: Aspirant's Ascent → DENTRO: **Exotic Orchard** (2026-07-31, voltron/protezione)
- [ ] FUORI: Frantic Search → DENTRO: **Arcane Denial** (2026-07-31, voltron/protezione)
- [ ] FUORI: Grapple with the Past → DENTRO: **Counterspell** (2026-07-31, voltron/protezione)
- [ ] FUORI: Windfall → DENTRO: **Lightning Greaves** (2026-07-31, voltron/protezione)
- [ ] FUORI: Monastery Siege → DENTRO: **My Precious // Allure of Power** (2026-07-31,
  voltron) — ⚠️ **da COMPRARE prima** (0 copie possedute, ~4,71€ Cardmarket), non è uno
  spostamento interno alla collezione come gli altri swap sopra

## deadpool (Sad Men Parade)

- [ ] FUORI: Grave Betrayal → DENTRO: **Terminate** (bulk, ECC)
- [ ] FUORI: The One Ring → DENTRO: **Burnished Hart** (2026-07-31, ceduto a favore di
  toph/yshtola — vedi deadpool_note.md sezione Contese; risolve anche ramp sotto target)

---

## Note

- Le carte FUORI tornano in `bulk` (o dove preferisci) — aggiorna lo scan Moxfield
  alla prossima sessione di scansione.
- `ultron.txt` ha perso Idol of Oblivion (101→100 carte) ma è un mazzo **teorico
  non montato**: nessuna azione fisica.
- Quando un nuovo swap viene deciso in chat, va aggiunto qui PRIMA di considerarlo
  concluso.
